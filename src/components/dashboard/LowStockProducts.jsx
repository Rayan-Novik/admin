...
const LowStockProducts = () => {
    ...
    useEffect(() => {
        const fetchLowStock = async () => {
            try {
                ...
                setError('Não foi possível carregar os produtos com baixo stock.');
            } finally {
                setLoading(false);
            }
        };
    ...
    return (
        <Card className="shadow-sm h-100">
            <Card.Body>
                <Card.Title>Alerta de Baixo Stock (&lt;10)</Card.Title>
                    ...
                    <ListGroup variant="flush">
                        {products.map(product => (
                            <ListGroup.Item key={product.id_produto} className="d-flex justify-content-between align-items-center">
                                ...
                                <Badge bg="danger" pill>{product.estoque} restantes</Badge>
                            </ListGroup.Item>
                        ))}
                    </ListGroup>
                )}
