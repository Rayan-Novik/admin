...
// --- Modal de Edição (AGORA COM SELEÇÃO INTELIGENTE DE DESTINO) ---
const DestaqueFormModal = ({ show, handleClose, onSave, destaque }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setLoadingData(true);
            try {
                // 🟢 Blindagem com .catch() para não quebrar a tela do modal se algo falhar
    ...
    useEffect(() => {
        if (destaque) { 
            setFormData({
                imagem_url: destaque.imagem_url || '',
                link_url: destaque.link_url || '',
                titulo: destaque.titulo || '',
                ativo: destaque.ativo !== undefined ? destaque.ativo : true,
                tipo_filtro: destaque.tipo_filtro || 'url',
                valor_filtro: destaque.valor_filtro || ''
            }); 
        } else { 
            setFormData({ 
                imagem_url: '', link_url: '', titulo: '', ativo: true,
            ...
            <Modal.Body className="p-4">
                {loadingData ? (
                    <div className="text-center py-5">
                        ...
                        <span className="text-muted">Carregando opções...</span>
                    </div>
                ) : (
                    <Form onSubmit={(e) => { e.preventDefault(); onSave(formData); }}>
                        ...
                        <Form.Group className="mb-3">
                            <Form.Label className="small fw-bold text-muted text-uppercase">Título</Form.Label>
                        ...
                        {/* --- SEÇÃO NOVA DE AÇÃO DE CLIQUE --- */}
                        <div className="bg-light p-3 rounded-3 mb-4">
                            <h6 className="fw-bold text-dark mb-3" style={{ fontSize: '0.85rem' }}>AÇÃO DE CLIQUE (DESTINO)</h6>
                            
                            <Form.Group className="mb-3">
                                <Form.Label className="small fw-bold text-muted text-uppercase" style={{ fontSize: '0.7rem' }}>Tipo</Form.Label>
                                <Form.Select 
                                    name="tipo_filtro" 
                                ...
                                >
                                    <option value="url">Link Personalizado (URL)</option>
                            ...
                            <Form.Group>
                                <Form.Label className="small fw-bold text-muted text-uppercase" style={{ fontSize: '0.7rem' }}>Selecione o Destino</Form.Label>
                                {formData.tipo_filtro === 'categoria' && (
                                    <Form.Select name="valor_filtro" value={formData.valor_filtro} onChange={handleValueSelectChange} required className="border-0 shadow-sm">
                                        <option value="">-- Selecione uma Categoria --</option>
                                        {categorias.map(cat => <option key={cat.id_categoria} value={cat.slug || cat.nome}>{cat.nome}</option>)}
                                    </Form.Select>
                                )}
                                {formData.tipo_filtro === 'subcategoria' && (
                                    <Form.Select name="valor_filtro" value={formData.valor_filtro} onChange={handleValueSelectChange} required className="border-0 shadow-sm">
                                        <option value="">-- Selecione uma Subcategoria --</option>
                                        {categorias.map(cat => (
                                            <optgroup key={cat.id_categoria} label={cat.nome}>
                                                {cat.subcategorias?.map(sub => <option key={sub.id_subcategoria} value={sub.slug || sub.nome}>{sub.nome}</option>)}
                                            </optgroup>
                                        ))}
                                    </Form.Select>
                                )}
                                ...
                                {formData.tipo_filtro === 'produto' && (
                                    <Form.Select name="valor_filtro" value={formData.valor_filtro} onChange={handleValueSelectChange} required className="border-0 shadow-sm">
                                        ...
                                        {produtos.map(p => <option key={p.id_produto} value={p.slug || p.id_produto}>{p.nome}</option>)}
                                    </Form.Select>
                                )}
                                {formData.tipo_filtro === 'url' && (
                                    <Form.Control type="text" name="link_url" value={formData.link_url} onChange={handleChange} placeholder="Ex: /categoria/ofertas" required className="border-0 shadow-sm" />
                                )}
                            </Form.Group>
                        </div>
                        {/* --- FIM DA SEÇÃO NOVA --- */}
                        ...
                        <div className="d-grid gap-2">
                            <Button variant="dark" type="submit" className="shadow-sm fw-bold rounded-pill py-2">Salvar Destaque</Button>
                        </div>
                    </Form>
                )}
            </Modal.Body>
...
const CategoryFeaturesManager = () => {
    ...
    const fetchDestaques = async () => {
        try {
            ...
            // 🟢 Blindagem na lista principal.
            // Ignora erro e deixa a lista vazia pra você poder criar o primeiro.
            setDestaques([]);
            console.log("Nenhum destaque encontrado ou acesso negado.");
        } finally { 
            setLoading(false); 
        }
    };
    ...
    // Opcional: Se quiser que o erro pare a tela inteira, você usa o setError. 
    // Como a gente blindou acima, o erro no backend apenas deixa a tela vazia, o que é mais suave.
                    ...
                    <Row className="g-2 justify-content-lg-end">
                        ...
                        <Col xs={12} sm={4}>
                            <Button 
                                ...
                                className="w-100 h-100 rounded-3 shadow-sm d-flex flex-column align-items-center justify-content-center p-0 fw-bold py-2"
                            >
