...
// --- Modal de Edição/Criação (AGORA COM SELEÇÃO INTELIGENTE DE DESTINO) ---
    ...
    useEffect(() => {
        if (slide) {
            setFormData({
                ...
                tipo_filtro: slide.tipo_filtro || 'url',
                valor_filtro: slide.valor_filtro || ''
            });
        } else {
            setFormData({ 
                imagem_url: '', link_url: '', titulo: '', subtitulo: '', 
                ativo: true, tipo_filtro: 'url', valor_filtro: '' 
            });
        }
    }, [slide]);
                ...
                {loadingData ? (
                    ...
                    <Form onSubmit={(e) => { e.preventDefault(); onSave(formData); }}>
                        <Row className="g-4">
                            ...
                            <Col md={12}><hr className="my-0 text-muted opacity-25" /></Col>
                            <Col md={12}><h6 className="text-primary mb-0 fw-bold">Ação de Clique (Destino do Banner)</h6></Col>

                            <Col md={6}>
                                <Form.Group>
                                    <Form.Label className="fw-bold small text-muted text-uppercase">Tipo de Destino</Form.Label>
                                    <Form.Select 
                                        name="tipo_filtro" 
                            ...
                            <Col md={6}>
                                <Form.Group>
                                    ...
                                    {formData.tipo_filtro === 'url' && (
                                        <Form.Control type="text" name="link_url" value={formData.link_url} onChange={handleChange} placeholder="Ex: /promocao ou https://..." required />
                                    )}
                                </Form.Group>
                            </Col>
                            {/* --- FIM DA SEÇÃO NOVA --- */}
                        ...
                        <div className="d-flex justify-content-end mt-4 pt-3 border-top gap-2">
                            ...
                            <Button variant="primary" type="submit" className="px-4 fw-bold shadow-sm">Salvar Slide</Button>
                        </div>
                    </Form>
                )}
            </Modal.Body>
...
const CarouselManager = () => {
    ...
    const fetchSlides = async () => {
        try {
            ...
            setSlides(data || []); // Se vier dados, salva
        } catch (err) { 
            // Se der erro 403 ou 404 (vazio), ignora e deixa a tela em branco para ele poder criar um novo
            setSlides([]); 
            console.log("Nenhum slide encontrado ou acesso bloqueado.");
        } finally { 
            setLoading(false); 
        }
    };
