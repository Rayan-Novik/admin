...
import { UserPlus, Search, UserCheck, ArrowLeft, Save, AlertCircle } from 'lucide-react'; // 🟢 Importamos o AlertCircle
...
export default function ClienteModal({ isOpen, onClose, onSelectCliente }) {
    ...
    const [errorMsg, setErrorMsg] = useState(''); // 🟢 ESTADO DE ERRO VISUAL
    ...
    const handleBuscar = async () => {
        ...
        try {
            ...
            // 🟢 AVISA SE FOR FALTA DE PERMISSÃO
            if (error.response?.status === 403) {
                setErrorMsg("🔒 Acesso Negado: Seu cargo não tem a permissão 'CLIENTES_MANAGE'.");
            } else {
                setErrorMsg("Erro ao buscar cliente. Verifique a conexão.");
            }
            setResultado([]);
        } finally {
            setLoading(false);
    ...
    const handleSalvarCliente = async () => {
        ...
        try {
            ...
            console.error(error);
            // 🟢 AVISA SE FOR FALTA DE PERMISSÃO NO CADASTRO
            if (error.response?.status === 403) {
                setErrorMsg("🔒 Acesso Negado: Você não tem permissão para cadastrar clientes.");
            } else {
                setErrorMsg("Erro ao cadastrar: " + (error.response?.data?.message || error.message));
            }
        } finally {
            ...
            <div className="modal-dialog modal-dialog-centered">
                <div className="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
                    ...
                    <div className="modal-body p-4 bg-light">
                        ...
                        {errorMsg && (
                            <div className="alert alert-danger d-flex align-items-center py-2 px-3 mb-4 rounded-3 border-0 shadow-sm" role="alert">
                                ...
                                <div className="list-group shadow-sm rounded-3 border-0 overflow-auto" style={{ maxHeight: '300px' }}>
                                    {loading ? (
                                        ...
                                        resultado.map(c => (
                                            <button
                                                key={c.id_usuario}
                                                onClick={() => { onSelectCliente(c); resetAndClose(); }}
                            ...
                            <form onSubmit={(e) => { e.preventDefault(); handleSalvarCliente(); }}>
                                ...
                                <div className="mt-4 pt-3 border-top d-flex gap-2">
                                    <button 
                                        ...
                                        onClick={() => { setView('search'); setErrorMsg(''); }}
                                    >
                    ...
                    {view === 'search' && (
                        <div className="modal-footer border-0 p-4 bg-white d-flex flex-column gap-2">
                            <button 
                                ...
                                onClick={() => { onSelectCliente(null); resetAndClose(); }}
                            >
