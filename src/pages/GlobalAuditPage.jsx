...
import { Container, Card, Table, Badge, Spinner, Form, InputGroup, Button, Alert } from 'react-bootstrap';
...
const GlobalAuditPage = () => {
    ...
    const fetchGlobalHistory = async () => {
        try {
            ...
            // 🟢 MEGAFONE LIGADO: Captura e exibe o erro exato na tela
            if (error.response?.status === 403) {
                setErrorMsg('🔒 Acesso Negado: A sua conta não tem permissão para visualizar a auditoria do sistema.');
            } else {
                setErrorMsg(`❌ Erro no Backend (${error.response?.status || 'Desconhecido'}): Não foi possível carregar o histórico.`);
        ...
        <Container fluid className="py-4 bg-light min-vh-100">
            <div className="d-flex flex-column flex-md-row justify-content-between align-items-center mb-4">
                ...
                <div className="d-flex gap-2 mt-3 mt-md-0">
                    <Button variant="outline-primary" onClick={fetchGlobalHistory} disabled={loading}>
                        {loading ? <Spinner size="sm" className="me-2" /> : <i className="bi bi-arrow-clockwise me-2"></i>}
                        Atualizar
                    </Button>
                </div>
            </div>

            {/* 🟢 ALERTA DE ERRO VISÍVEL NA TELA */}
            {errorMsg && (
                <Alert variant="danger" className="text-center rounded-4 fw-bold shadow-sm">
                    {errorMsg}
                </Alert>
            )}
            ...
            {!errorMsg && (
                <Card className="border-0 shadow-sm">
                    ...
                    <Card.Footer className="bg-white text-muted small text-end border-top-0">
