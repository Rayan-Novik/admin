...
import { 
    Plus, Search, CheckCircle, AlertCircle, FileText 
} from 'lucide-react';
...
export default function FinancialModule() {
    ...
    const fetchDashboard = useCallback(async () => {
        try {
            ...
            if (error.response?.status === 403) {
                setErrorMsg('🔒 Acesso Negado ao Dashboard.');
            }
        }
    }, [filters.startDate, filters.endDate]);

    const fetchAccounts = useCallback(async () => {
        if (activeTab === 'dre') return; // Se for aba DRE, não busca contas
        ...
        try {
            ...
            if (filters.status && filters.status !== 'TODOS') {
                paramsToSend.status = filters.status;
            }

            const res = await api.get(endpoint, { params: paramsToSend });
            
            let arrayDeContas = [];
            if (Array.isArray(res.data)) arrayDeContas = res.data; 
            else if (res.data && Array.isArray(res.data.data)) arrayDeContas = res.data.data; 
            else if (res.data && Array.isArray(res.data.contas)) arrayDeContas = res.data.contas; 
            else if (res.data && Array.isArray(res.data.payables)) arrayDeContas = res.data.payables;
            else if (res.data && Array.isArray(res.data.receivables)) arrayDeContas = res.data.receivables;
            
            const sortedData = arrayDeContas.sort((a, b) => {
            ...
            if (error.response?.status === 403) {
                setErrorMsg('🔒 Acesso Negado: Sua conta não tem a permissão "CONTAS_MANAGE".');
            } else {
                setErrorMsg(`❌ Erro no Banco de Dados: ${error.response?.data?.message || error.message}`);
            }
    ...
    const fetchDre = useCallback(async () => {
        ...
        try {
            ...
            toast.error("Não foi possível carregar o relatório DRE.");
        } finally {
            setLoadingDre(false);
        }
    }, [filters.startDate, filters.endDate]);
    ...
    }, [fetchDashboard, fetchAccounts, fetchDre, activeTab]);
    ...
    const CreateAccountModal = () => {
        const [formData, setFormData] = useState({
            descricao: '', valor_total: '', data_vencimento: getLocalDateString(today),
        ...
        const handleSubmit = async (e) => {
            e.preventDefault();
            try {
                if (activeTab === 'receivables') return alert("Criação bloqueada para recebíveis.");
                const payload = {
                    ...formData,
        ...
        return (
            <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)}>
                <Modal.Header closeButton><Modal.Title>Nova Conta a Pagar</Modal.Title></Modal.Header>
                <Form onSubmit={handleSubmit}>
                    <Modal.Body>
                        <Form.Group className="mb-3">
                            <Form.Label>Descrição</Form.Label>
                            <Form.Control required type="text" onChange={e => setFormData({...formData, descricao: e.target.value})} />
                        </Form.Group>
                        <Row>
                            <Col>
                                <Form.Group className="mb-3">
    ...
    const SettleModal = () => {
        const [baixaData, setBaixaData] = useState({
            valor: 0, data_baixa: getLocalDateString(today),
        ...
        return (
            <Modal show={showSettleModal} onHide={() => setShowSettleModal(false)}>
                <Modal.Header closeButton className={activeTab === 'payables' ? 'bg-danger text-white' : 'bg-success text-white'}>
                    <Modal.Title>{activeTab === 'payables' ? 'Pagar Conta' : 'Receber Conta'} #{selectedAccount.id}</Modal.Title>
                </Modal.Header>
    ...
    const NoteModal = () => {
        ...
        return (
            <Modal show={showNoteModal} onHide={() => setShowNoteModal(false)} size="lg">
                <Modal.Header closeButton><Modal.Title>Detalhes</Modal.Title></Modal.Header>
                <Modal.Body>
                    {loadingNote ? (
                        ...
                        <>
                            ...
                            {transacoes.length === 0 ? (
                                <Alert variant="warning">Nenhum pagamento registrado.</Alert>
                            ) : (
                                <Table striped bordered hover size="sm" responsive className="small">
                                    <thead className="table-dark">
                                        <tr><th>Data</th><th>Forma</th><th className="text-end">Bruto</th><th className="text-end">Taxas</th><th className="text-end">Líquido</th></tr>
                                    </thead>
                                    <tbody>
        ...
        <Container fluid className="py-4 bg-light min-vh-100">
            <div className="d-flex justify-content-between align-items-center mb-4">
                <div>
                    ...
                    <small className="text-muted">Fluxo de Caixa, Contas e DRE</small>
                </div>
                {activeTab === 'payables' && !errorMsg && (
            ...
            {errorMsg && (
                <Alert variant="danger" className="text-center rounded-4 fw-bold shadow-sm mb-4">
                    <AlertCircle className="me-2 mb-1" size={24} /> {errorMsg}
                </Alert>
            )}
                ...
                <>
                    {dashboard && dashboard.resumo && (
                        <Row className="g-3 mb-4">
                            <Col md={3}><Card className="border-0 shadow-sm border-start border-4 border-success"><Card.Body><small className="text-muted fw-bold">ENTRADAS</small><h4 className="fw-bold text-success mb-0">{formatCurrency(dashboard.resumo.total_entradas)}</h4></Card.Body></Card></Col>
                            <Col md={3}><Card className="border-0 shadow-sm border-start border-4 border-danger"><Card.Body><small className="text-muted fw-bold">SAÍDAS</small><h4 className="fw-bold text-danger mb-0">{formatCurrency(dashboard.resumo.total_saidas)}</h4></Card.Body></Card></Col>
                            <Col md={3}><Card className={`border-0 shadow-sm border-start border-4 ${dashboard.resumo.saldo_periodo >= 0 ? 'border-primary' : 'border-warning'}`}><Card.Body><small className="text-muted fw-bold">SALDO LÍQUIDO</small><h4 className={`fw-bold mb-0 ${dashboard.resumo.saldo_periodo >= 0 ? 'text-primary' : 'text-warning'}`}>{formatCurrency(dashboard.resumo.saldo_periodo)}</h4></Card.Body></Card></Col>
                            <Col md={3}><Card className="border-0 shadow-sm bg-danger text-white"><Card.Body><div className="d-flex justify-content-between"><div><small className="text-white-50 fw-bold">ATRASADAS</small><h4 className="fw-bold mb-0">{formatCurrency(dashboard.resumo.divida_atrasada)}</h4></div></div></Card.Body></Card></Col>
                        </Row>
                    )}
                    ...
                    <Card className="border-0 shadow-sm">
                        ...
                        {/* --- RENDERIZAÇÃO CONDICIONAL: DRE vs CONTAS --- */}
                        {activeTab === 'dre' ? (
                            loadingDre ? (
                                ...
                                <div className="table-responsive p-3">
                                    <Table hover className="align-middle mb-0 border">
                                        <thead className="bg-light">
                                            <tr>
                                                <th className="ps-4">Categoria Financeira</th>
                                                <th className="text-end text-success">Receitas (+)</th>
                                                <th className="text-end text-danger">Despesas (-)</th>
                                                <th className="text-end pe-4">Saldo Consolidado</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            ...
                                            <tr className="bg-light border-top border-2">
                                                <td className="ps-4 fw-bold text-uppercase h6 pt-4 pb-4 mb-0">Resultado Líquido do Período</td>
                                                    ...
                                                    <tr key={conta.id} className={isLate ? 'table-danger' : ''}>
                                                        ...
                                                        <td>
                                                            <div className="d-flex align-items-center gap-2">
                                                                <span className={isLate ? 'text-danger fw-bold' : (isToday ? 'text-warning fw-bold' : '')}>{formatDate(conta.data_vencimento)}</span>
                                                                {isLate && <Badge bg="danger">-{Math.abs(daysDiff)}d</Badge>}
                                                                {isToday && <Badge bg="warning" text="dark">Hoje</Badge>}
                                                            </div>
                                                        </td>
                                                        <td>{conta.parcela_total > 1 ? <Badge bg="secondary">{conta.parcela_numero}/{conta.parcela_total}</Badge> : <span className="text-muted">-</span>}</td>
                                                        ...
                                                        <td className="text-end pe-4">
                                                            <div className="d-flex justify-content-end gap-2">
                                                                <Button variant="outline-secondary" size="sm" onClick={() => handleOpenNoteModal(conta)}><FileText size={16} /></Button>
                                                                {conta.status !== 'PAGO' && conta.status !== 'CANCELADO' && (
                                                                    <Button size="sm" variant={activeTab === 'payables' ? 'outline-danger' : 'outline-success'} onClick={() => { setSelectedAccount(conta); setShowSettleModal(true); }}>
                                                                        <CheckCircle size={16} className="me-1" />{activeTab === 'payables' ? 'Pagar' : 'Receber'}
                                                                    </Button>
                                                                )}
                                                            </div>
                                                        </td>
